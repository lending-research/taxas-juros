...
def fetch_bacen(data_inicio: str, data_fim: str) -> list:
    ...
    # Sem $filter na URL — a API do Bacen rejeita filtros complexos via OData.
    # Buscamos todos os dados do período e filtramos em Python.
    url = (
        f"{base}"
        f"?$format=json"
        f"&$select=InicioPeriodo,InstituicaoFinanceira,TaxaJurosAoMes,TaxaJurosAoAno,Posicao,Segmento,Modalidade"
        f"&$orderby=InicioPeriodo%20asc,Posicao%20asc"
        f"&$top=5000"
    ...
    # Filtra por Modalidade e Segmento em Python
    ...
    print(f"  Total recebido: {len(all_records)} | Após filtro: {len(filtered)}")
    return filtered
